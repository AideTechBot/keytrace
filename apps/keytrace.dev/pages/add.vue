    ...
    <Transition name="slide" mode="out-in">
      <div v-if="currentStep === 0" key="step-0">
        ...
        <p class="mt-2 text-zinc-400 text-sm">
          Choose an account or service to link to your identity.
        </p>
      ...
      <div v-else-if="currentStep === 1" key="step-1">
        ...
        <ol class="mt-6 space-y-4">
          <li
            v-for="(instruction, i) in selectedInstructions"
            :key="i"
            class="flex gap-3"
          >
        ...
        <div class="mt-6">
          <KtInput
            v-model="claimUri"
            :label="selectedService?.inputLabel ?? 'Claim URL'"
            :placeholder="selectedService?.inputPlaceholder ?? 'https://...'"
          />
      ...
      <div v-else-if="currentStep === 2" key="step-2">
        <h2 class="text-2xl font-semibold text-zinc-100 tracking-tight mb-6">
        ...
        <div v-if="verificationComplete && verificationSuccess" class="text-center py-8">
          ...
          <p class="text-sm text-zinc-400 mt-2">
            Your identity proof has been verified and stored in your ATProto repo.
          </p>
          <div class="mt-6 flex items-center justify-center gap-3">
            <NuxtLink
              to="/dashboard"
              class="px-5 py-2 bg-violet-600 hover:bg-violet-500 text-white text-sm font-medium rounded-lg transition-all"
            >
              Go to Dashboard
            </NuxtLink>
            <button
              class="px-5 py-2 text-sm text-zinc-400 hover:text-zinc-200 transition-colors"
              @click="reset"
            >
              Add another
            </button>

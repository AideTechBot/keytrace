    ...
    >
      <template #user>
        <div v-if="session?.authenticated" class="relative" ref="menuRef">
          <button
            class="w-8 h-8 rounded-full bg-zinc-800 overflow-hidden flex items-center justify-center cursor-pointer"
          ...
          >
            <div
              v-if="menuOpen"
              class="absolute right-0 mt-2 min-w-[180px] rounded-lg bg-zinc-900 border border-zinc-800 shadow-xl py-1 z-50 origin-top-right"
            >
              <NuxtLink
                :to="`/${session.handle}`"
                class="flex items-center gap-2 px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-800 hover:text-zinc-100"
              ...
              <button
                class="flex items-center gap-2 px-3 py-2 text-sm text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100 w-full"
                @click="handleLogout"
              >
